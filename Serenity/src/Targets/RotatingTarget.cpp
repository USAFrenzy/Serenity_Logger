...
// clang-format off
	...
	void RotatingTarget::RotateOnSize(int duration) {
		while (!cleanUpRotatingThread.load()) {
			if (IsIntervalRotationEnabled()) {
				if ((FileSize() + MsgInfo()->MessageSize()) >= fileSizeLimit) {
					RotateFile();
				}
			}
	...
	void RotatingTarget::RotateHourly(int duration) {
		auto& cache{ MsgInfo()->TimeDetails().Cache() };
		while (!cleanUpRotatingThread.load()) {
			if (currentHour != cache.tm_hour) {
				currentHour = cache.tm_hour;
	...
	void RotatingTarget::RotateDaily(int duration) {
		auto& cache{ MsgInfo()->TimeDetails().Cache() };
		while (!cleanUpRotatingThread.load()) {
			if (currentDay != cache.tm_mday) {
				currentDay = cache.tm_mday;
			...
			if (IsIntervalRotationEnabled()) {
				if ((dayModeSettingHour == cache.tm_hour) || (cache.tm_min >= dayModeSettingMinute)) {
					RotateFile();
					EnableFirstRotation(false);
				}
			}
	...
	void RotatingTarget::SetRotationSetting(IntervalMode mode, int duration, int  setting, int secondSetting)
	{
		std::unique_lock<std::mutex> lock(rotatingMutex, std::defer_lock);
		if (isMTSupportEnabled()) {
			lock.lock();
		...
		case mType::file_size:
			...
			if (rotatingThreadEnabled.load()) {
				cleanUpRotatingThread.store(true);
				rotatingThread.join();
			}
			rotatingThread = std::thread(&RotatingTarget::RotateOnSize, this);
			rotatingThreadEnabled.store(true);
			break;
		case mType::hourly:
			if (rotatingThreadEnabled.load()) {
				cleanUpRotatingThread.store(true);
			...
			rotatingThread = std::thread(&RotatingTarget::RotateHourly, this);
			rotatingThreadEnabled.store(true);
			break;
		case mType::daily:
		{
			...
			rotatingThread = std::thread(&RotatingTarget::RotateDaily, this);
			rotatingThreadEnabled.store(true);
		} break;
		case mType::weekly:
		{
