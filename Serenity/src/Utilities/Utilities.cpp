	...
	namespace file_utils {
		...
		file_utils_results::retrieve_dir_entries const RetrieveDirEntries(std::filesystem::path& path, bool recursive) {
			...
			if( path.has_extension() ) {
					printf("ERROR: Path To Retrieve Directory Entries Points To A File Instead "
					       "Of A Directory\nPath: "
					       "%s\n",
					       path.string().c_str());
		...
		bool OpenFile(std::filesystem::path file, bool truncate) {
			...
			int mode;
			// By Default Create A File With Full Permissions If File Doesn't Already
			// Exist
			if( !fs::exists(file) ) {
					try {
					...
					// By Default, Only Checking If The File Has Any Permissions To Open (Could
					// Use Work Here)
					if( fs::status(file).permissions() == (fs::perms::none) ) {
							printf("Error In OpenFile():\nUnable To Open File Due To Insufficient "
							       "Permissions.");
							return false;
					}
				}
