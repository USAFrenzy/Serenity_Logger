...
#include <serenity/Defines.h>
		...
		#define VFORMAT_TO(cont, msg, ...)        std::vformat_to<CONTEXT>(std::back_inserter(cont), msg, std::make_format_args(__VA_ARGS__))
		#define VFORMAT_TO_L(cont, loc, msg, ...) std::vformat_to<CONTEXT>(std::back_inserter(cont), loc, msg, std::make_format_args(__VA_ARGS__))
			...
			#define VFORMAT_TO_L(cont, loc, msg, ...) std::format_to(std::back_inserter(cont), loc, msg, std::make_format_args(__VA_ARGS__))
		#else
	...
	#define CONTEXT                           std::back_insert_iterator<std::basic_string<char>>
	#define VFORMAT_TO(cont, msg, ...)        fmt::vformat_to<CONTEXT>(std::back_inserter(cont), msg, fmt::make_format_args(__VA_ARGS__))
	#define VFORMAT_TO_L(cont, loc, msg, ...) fmt::vformat_to<CONTEXT>(std::back_inserter(cont), loc, msg, fmt::make_format_args(__VA_ARGS__))
