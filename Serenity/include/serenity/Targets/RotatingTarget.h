...
namespace serenity::experimental::targets {
	class RotatingTarget: public serenity::targets::FileTarget, serenity::experimental::RotateSettings
	{
		public:
			...
			// on (0-23) For IntervalMode::weekly -> interval is the weekday that the
			// logger should rotate on (0-6) For IntervalMode::monthly -> interval is the
			// day of the month the logger should rotate on (1-31) EXAMPLES:
			// - SetRotationSetting( IntervalMode::daily, 23); -> rotate every day at 11PM
			// - SetRotationSetting(IntervalMode::file_size, 512*KB) -> rotate when file
			// hits 512KB
			// - SetRotationSetting(IntervalMode::weekly, 2) -> Rotate Every Tuesday
			// - SetRotationSetting(IntervalMode::monthly, 24) -> Rotate Every Month on
			// the 24th day of that month This gives granularity over which setting to
			// change vs all settings in SetRotateSettings();
