...
namespace serenity::experimental::targets {
	...
	// Duration parameter denotes the interval to check the condition
	...
	// - SetRotationSetting(IntervalMode::monthly, 24) -> Rotate Every Month on the 24th day of that month.
		void SetRotationSetting(IntervalMode mode, int duration, int setting = 0, int secondSetting = 0);
		// clang-format on	
	...
	protected:
		...
		void RotateOnSize(int duration);
		void RotateHourly(int duration);
		void RotateDaily(int duration);
	...
	private:
		...
		std::atomic<bool> rotatingThreadEnabled{ false };
		std::atomic<bool> cleanUpRotatingThread{ false };
		std::thread rotatingThread;
	};
