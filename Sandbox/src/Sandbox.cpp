...
#define ParseFormatStringType_TESTING 1
...
#if ParseFormatStringType_TESTING
	#define ENABLE_ParseFormatStringType_SECTION
...
int main() {
...
#ifdef ENABLE_ParseFormatStringType_SECTION

	using namespace serenity::arg_formatter;
	constexpr std::string_view ParseFormatStringTypeString { "\n{0:*^#{5}X}\n{1:*^#{5}x}\n{2:*^#{5}a}\n{3:*^#{5}E}\n{4:*^#{5}b}\n{5:*^#{5}B}\n{6:s}" };
	...
	ArgFormatter ParseFormatStringTyper;
	...
	for( int i { 0 }; i < 3; ++i ) {
			...
			for( size_t i { 0 }; i < 10'000'000; ++i ) {
					ParseFormatStringTyper.se_format_to(std::back_inserter(finalStr), ParseFormatStringTypeString, a, b, c, d, e, f, tmp);
					finalStr.clear();
				}
			...
			timer.StopWatch_Reset();
			for( size_t i { 0 }; i < 10'000'000; ++i ) {
					finalStr = ParseFormatStringTyper.se_format(ParseFormatStringTypeString, a, b, c, d, e, f, tmp);
				}
			timer.StopWatch_Stop();
			finalStr.clear();
			...
			for( size_t i { 0 }; i < 10'000'000; ++i ) {
					L_VFORMAT_TO(finalStr, locale, ParseFormatStringTypeString, a, b, c, d, e, f, tmp);
					finalStr.clear();
				}
			timer.StopWatch_Stop();
			finalStr.clear();
...
#endif    // ENABLE_ParseFormatStringType_SECTION
